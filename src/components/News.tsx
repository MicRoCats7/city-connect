...
function News() {
    ...
    return (
        <section className="desktop:mt-[800px] tablet:mt-[800px] mt-[400px]" id={t('navbar.news')}>
            <div className="max-w-[1440px] mx-auto px-4 tablet:px-12 desktop:px-36">
                <div className="flex items-center gap-4">
                    <h2 className="text-3xl desktop:text-[50px] font-general-sans-semibold special-text">{t("news.title")}</h2>
                                ...
                                return (
                                    <div key={news.id} className="relative col-span-1 flex flex-col justify-between cursor-pointer bg-transparent hover:scale-90 transition transform">
                                        <img
                                            ...
                                            alt={news.title}
                                        />
                                        <p className="desktop:mb-5 mb-2 text-sm desktop:text-[22px] leading-6 font-general-sans-semibold text-white">{news.title}</p>
                                        <p className="font-general-sans-medium text-xs desktop:text-base opacity-60 text-white">{news.date}</p>
                                    </div>
                                )
                            } else {
                                return (
                                    <div key={news.id} className="relative col-span-2 row-span-2 flex flex-col justify-between cursor-pointer hover:scale-90 transition transform">
                                        <img
                                            ...
                                            alt={news.title}
                                        />
                                        <div className="absolute desktop:bottom-0 bottom-5 w-full p-5 bg-gradient-to-t from-black/50 to-transparent">
